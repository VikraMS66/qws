...
<section  id="searchorders" class="pt-5">
<?php 
            if (isset($_POST['searchValue'])) {
                $value = mysqli_real_escape_string($conn, $_POST['searchValue']);
?>
 <div class="mt-4">
    <form method='post' action='homepage.php?info=search_orders'>
       <div class='row'>
           <div class='col-3'>
     ...
     <?php 
            ...
            $ID = $_SESSION['sellerid'] ;
              ...
              $sql_query="SELECT * FROM orders WHERE supid='$ID' AND (id ='$value' OR status LIKE '%".$value."%' OR pincode='$value' OR custName LIKE '%".$value."%' OR custNumber='$value' OR supName LIKE '%".$value."%' OR supNumber='$value' OR product='$value') ORDER BY date DESC";
