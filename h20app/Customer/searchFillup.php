 ...
 //include("auth.php");
  ...
  if (isset($_POST['searchPin'])) {
    $pin = mysqli_real_escape_string($conn, $_POST['searchPin']);
      if(strlen($pin) > 0) {
      $query="SELECT * FROM fillup where status='Active' AND pincode LIKE '%".$pin."%'";
      } else {
        $query="SELECT * FROM fillup where status='Active'";
      }
     $res=mysqli_query($conn, $query);
  ?>
    <div class="mt-5 pt-5 text-start mx-3 mb-5 pb-3 bg-info">
   
      <form method='post' action='cHomePage.php?info=searchfillup'>
        <div class="input-group text-center mx-3 mb-3" style="width:300px;">
         <input name="searchPin"  oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" onkeypress="return onlyNumberKey(event)"  id="pincodeEntered" type="number" class="form-control rounded ml-5" style="width:50px;" maxlength="6" minlength="6" placeholder="Enter Pincode to Search" value='<?php echo $pin ?>'/>
