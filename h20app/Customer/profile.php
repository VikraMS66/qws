<?php 
     ...
     $sql_query="SELECT * FROM customer WHERE mNumber='$ID'";
                ...
                <div class="row d-flex justify-content-center align-items-center h-100">
                 <div class="col-lg-10 col-xl-6">
                    ...
                    <div class="card-body p-4 p-md-5">
                        <h3 class="mb-4 pb-2 pb-md-0 mb-md-5 px-md-2">Profile Info</h3>

                        <form class="px-md-2" method='post'>

                        <div class="form-outline mb-4">
                            <label class="form-label" for="form3Example1q">Name</label>
                            <input type="text" id="form3Example1q" class="form-control" name='username' value='<?php echo $name ?>'/>
                            
                        </div>
                        <div class="form-outline mb-4">
                            <label class="form-label" for="form3Example1qa">Eamil</label>
                            <input type="email" id="form3Example1qa" class="form-control" name='email' value='<?php echo $email ?>'/>
                            
                        </div>
                        <div class="form-outline mb-4">
                             <label class="form-label" for="form3Example1qb">Mobile Number</label>
                            <input type="number" id="form3Example1qb" readonly class="form-control" name='number' value='<?php echo $number ?>'/>
                            
                        </div>
                         <div class='row mt-2 mb-0'>
                            <div class="col-6">
                                <button type="submit" style='width:60px;' id="editProfile" name='profileEdit' class="btn btn-info mb-1">Save</button>
                            </div>
                            <div class="col-6">
                ...
                if (isset($_POST['passChange'])) {
                    ...
                    if($pass == $current) {
                        ...
                        if($pwd != $repwd) {
                            ...
                            $sql_query="update customer set password='$repwd' where mNumber='$number'";
                            $res=mysqli_query($conn,$sql_query);
                ...
                if (isset($_POST['profileEdit'])) {
                    ...
                    $email = mysqli_real_escape_string($conn, $_POST['email']);
                     if(strlen($name) > 0 && strlen($email) > 0){
                            ...
                            $sql_query="update customer set name='$name', email='$email' where mNumber='$number'";
                            $res=mysqli_query($conn,$sql_query);
                            if($res) {
                                echo("<script>alert('Profile Edited!');</script>");
                                echo("<script>location.href = 'cHomePage.php?info=profile';</script>");
                             }
                        } else {
