     ...
     <div class='row'>
      ...
      <form method='post' action='associate.php?info=searchfillup'>
        <div class="input-group text-center mx-3 mb-3" style="width:300px;">
         <input name="searchPin"  oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" onkeypress="return onlyNumberKey(event)"  id="pincodeEntered" type="text" class="form-control rounded ml-5" style="width:50px;" maxlength="6" minlength="6" placeholder="Enter Pincode"/>
          <button type="submit" id="searchOrder" class="btn btn-outline-dark text-white rounded">Search</button>
        </div>
      </form>
      </div>
      <div class='col-2'>
      <a type="button" href='associate.php?info=addFillup' id="addFillup" class="btn btn-outline-dark text-end text-info rounded">Add New Fill-up Point</a>
      </div>
      </div>
