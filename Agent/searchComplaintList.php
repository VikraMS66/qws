...
?>
 ...
 <div class="mt-4">
     <form method='post' action='associate.php?info=search_complaint'>
     <div class='row'>
           <div class='col-4'>
               <div class="input-group text-center mx-3 mb-2" style="width:300px;">
                   <?php
                   echo "<input name='searchValue' oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' onkeypress='return onlyNumberKey(event)'  id='searchValue' type='text' class='form-control rounded' maxlength='20' minlength='1' value='".$value."' placeholder='Enter number' />";
                   ?>
...
<?php 
     ...
     if(strlen($value) > 0) {
        ...
        $sql_query="SELECT * FROM complaint WHERE id='$value' OR orderid='$value' OR number='$value' ORDER BY date DESC";

        } else {
  
          $sql_query="SELECT * FROM complaint ORDER BY date DESC";
...
$.post(
   ...
   setTimeout(redirectPage, 300);
     ...
     location.href='associate.php?info=complaintDetails';
     }
});
