...
?>
 ...
 <div class="mt-4">
    <form method='post' action='associate.php?info=csearch_orders'>
       <div class='row'>
           <div class='col-3'>
           ...
           <div class='col-9 text-dark h5'>
                 <p><u>To Search use OrderID or Order Status or Pincode or Delivery Address (Name or Number) or Supplier (Name or Number) or Capacity.</u></p>
           </div>
       </div>
    </form>
  </div>
     <?php 
                ...
                $sql_fetch="SELECT * FROM agent WHERE number='$ID'";
                $res_get=mysqli_query($conn,$sql_fetch);

                if (mysqli_num_rows($res_get) == 1) {
                while($row_get = mysqli_fetch_assoc($res_get)) {
                  $custid = $row_get['selectedid'];
                }
                }
          
              $sql_query="SELECT * FROM orders WHERE custid='$custid' AND (id ='$value' OR status LIKE '%".$value."%' OR pincode='$value' OR custName LIKE '%".$value."%' OR custNumber='$value' OR supName LIKE '%".$value."%' OR supNumber='$value' OR product='$value') ORDER BY date DESC";
               ...
               <div class="card border border-white">
                 <div class="card-body bg-white text-dark">
                   ...
                   <div class="row bg-dark text-white pt-2">
                     <div class="col-4">
                      <?php
                        echo "<button tyle='width:100px;' id='".$row['id']."' class='btn btn-warning mb-2 complaint'>Complaint</button>";
                       ?>
                        </div>
                       <div class="col-4">
                         <?php
                      ...
                      <?php 
                       if($row['status'] === 'Delivered' || $row['status'] === 'Cancelled' || $row['status'] === 'OnWay') {
...
$(".save").click(function (event) {
   ...
   // alert(status);
      ...
      $.post(
           'http://localhost:8013/h20app/Agent/OrderStatus.php',
 ...
 $(".complaint").click(function(event) {
   ...
   // alert(status);
      ...
      $.post(
           'http://localhost:8013/h20app/Agent/setOrderid.php',
           { id: id },
             function(id) {
             console.log(id);
           }
          ...
          function redirectPage(){
            location.href='associate.php?info=cComplaint';
          }
